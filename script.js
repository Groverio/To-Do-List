...
function addTodo() {
  ...
  const now = new Date();
  if (new Date(date) < now.setHours(0, 0, 0, 0)) {
    alert('Please select the current date or a future date.');
    return;
  }

  // Check that time is not in the past if date is today
  if (date === inputDateElement.min && time < inputTimeElement.min) {
...
// Ensure you have Font Awesome included in your HTML for the icons
